...
class DownstreamTrainer:
    ...
    def __init__(
        ...
        :param imbalance_ratio: imbalance ratio
    ...
    def initiate_writer(self) -> None:
        ...
        os.system(
            "cp configs/downstream_config.yaml " +
            self.model_out_dir + "/config.yaml"
        )
    ...
    def train(self, epochs: int = 100) -> None:
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, (data, target, _) in loop:
    ...
    def test(
        ...
        # write probabilities and correct labels to numpy file
        np.save(
            self.model_out_dir + "/probabilities.npy",
            np.array(probabilitise)
        )
        np.save(
            self.model_out_dir + "/correct.npy",
            np.array(correct)
        )
        np.save(
            self.model_out_dir + "/predictions.npy",
            np.array(predict)
        )
    ...
    def data_loader(
        ...
        #     # then add majority class samples to increase the
        #     # imbalance ratio to the desired value

        #     nat_indices = np.where(dataset.targets == 1)[0]
        #     wat_indices = np.where(dataset.targets == 0)[0]

        #     if len(nat_indices) > len(wat_indices):
        #         majority_indices = nat_indices
        #         minority_indices = wat_indices
        #     else:
        #         majority_indices = wat_indices
        #         minority_indices = nat_indices

        #     majority_indices, extra_majority_indices = majority_indices[:len(
        #         minority_indices)], majority_indices[len(minority_indices):]

        #     # take extra samples from the majority class to increase the
        #     # imbalance ratio to the desired value
        #     upsample_factor = int(
        #         len(minority_indices) * (imbalance_ratio - 1)
        #     )
        #     upsampled_majority_indices = np.random.choice(
        #         extra_majority_indices, size=upsample_factor, replace=False
        # )

        # split the dataset into train and test
        train_dataset, test_dataset = train_test_split(
            dataset, test_size=0.1,
            random_state=2171, stratify=dataset.targets
        )
        ...
        #         int(imbalance_ratio * sum(class_counts))

        #     upsample_factor = desired_minority_count - minority_count
        #     minority_indices = np.where(
        #         train_targets == np.argmin(class_counts)
        #     )[0]
        #     upsampled_minority_indices = np.random.choice(
        #         minority_indices, size=upsample_factor, replace=True
        #     )

        #     new_dataset = torch.utils.data.Subset(
        #         train_dataset,
        #         upsampled_minority_indices
        #     )
