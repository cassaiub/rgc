...
class BentTailDataset(Dataset):
    ...
    def __init__(
        ...
        # take 50%-50% for both classes
        if self.include_small:
            nat_indices = np.where(self.targets == 1)[0]
            wat_indices = np.where(self.targets == 0)[0]

            wat_indices = np.random.choice(wat_indices, len(nat_indices), replace=False)

            indices = np.concatenate((nat_indices, wat_indices))
    ...
    def __getitem__(self, index: int) -> tuple:
        ...
        return data, target, self.filenames[index]
    ...
    def _load_data(self) -> None:
        ...
        self.filenames = np.array(
            [os.path.basename(path) for path in os.listdir(data_dir)]
        )
...
class AugmentedDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        :rtype: torch.Tensor, torch.Tensor, str
        """
        image, label, filename = self.dataset[index]
...
if __name__ == "__main__":
    ...
    print(f"Number of NAT: {len(nat_indices)}")
    print(f"Number of WAT: {len(wat_indices)}")
